...
def analyze_pr_changes(base_ref: str, head_ref: str) -> ChangeAnalysis:
    ...
    translation_files = [
        ...
        if "locale" in f and (f.endswith(".po") or f.endswith(".pot"))
    ]
        ...
        if diff_exit_code == 0:
            ...
            for line in diff_output.split("\n"):
                if line.startswith("+") and any(
                    pattern in line for pattern in translation_patterns
                ):
...
def validate_po_file(po_file: Path) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        non_header_empty = sum(
            1 for msgid in msgids if msgid and msgid in empty_msgstrs
        )
        ...
        output_github_actions_format(
            {
                ...
                "translation_files_changed": str(
                    len(analysis.translation_files_changed)
                ),
                "has_translatable_changes": str(
                    analysis.has_translatable_changes
                ).lower(),
