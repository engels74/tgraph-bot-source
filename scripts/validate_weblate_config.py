...
def validate_weblate_config(config_path: Path | None = None) -> bool:
    ...
    try:
        ...
        # Validate components
        components = [section for section in config.sections()  # pyright: ignore[reportAny]
        ...
        for component_section in components:  # pyright: ignore[reportAny]
            ...
            for field in required_fields:
                if field not in component:
                    logger.error(f"Missing required field '{field}' in component {component_name}")
                    all_valid = False
                    continue
...
def main() -> int:
    ...
    try:
        ...
        if config_valid and locale_valid:
            ...
            logger.info("You can now set up your project on Weblate using this configuration.")
            return 0
        else:
            logger.error("❌ Weblate configuration has issues that need to be addressed.")
            return 1
