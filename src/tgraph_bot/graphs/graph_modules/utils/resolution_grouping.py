...
def group_resolution_by_strategy(resolution: str, strategy: str) -> str:
    ...
    if strategy == "simplified":
        ...
        logger.warning(
            f"Invalid grouping strategy '{strategy}', falling back to 'detailed'"
        )
        return _format_detailed(resolution)
    ...
    for target_ratio, tolerance in video_ratios:
        if abs(aspect_ratio - target_ratio) <= tolerance:
            ...
            if abs(target_ratio - 4 / 3) < 0.01:  # This is 4:3 ratio
                ...
                return (
                    width <= 720 and height <= 576
                )  # Exclude XGA and higher computer resolutions
            return True
...
def _group_standard(resolution: str) -> str:
    ...
    # But exclude computer display ratios like 1600x1200
        ...
        1440 <= width <= 1600 and _is_reasonable_video_aspect_ratio(width, height)
    ):
        return "1440p"
...
def aggregate_by_resolution_grouped(
    ...
    # Log statistics
    logger.info(
        f"Grouped resolution aggregation for field '{resolution_field}' with strategy '{grouping_strategy}': "
...
def aggregate_by_resolution_and_stream_type_grouped(
    ...
    resolution_stream_counts: defaultdict[str, defaultdict[str, int]] = defaultdict(
    ...
    # Log statistics
    logger.info(
        f"Grouped resolution and stream type aggregation for field '{resolution_field}' with strategy '{grouping_strategy}': "
...
def _sort_aggregates_by_quality_and_count(
    aggregates: "ResolutionAggregates",
) -> "ResolutionAggregates":
    ...
    return sorted(
        aggregates,
        key=lambda x: (quality_order.get(x["resolution"], 999), -x["play_count"]),
    )
...
def _sort_resolution_stream_aggregates_by_quality(
    ...
    sorted_resolutions = sorted(
        result.keys(), key=lambda x: (quality_order.get(x, 999), -resolution_totals[x])
    )
