...
class DataFetcher:
    ...
    def __init__(
        self, base_url: str, api_key: str, timeout: float = 30.0, max_retries: int = 3
    ) -> None:
        """
    ...
    async def _make_request(
        self, endpoint: str, params: dict[str, str | int | float | bool] | None = None
    ) -> TautulliData:
        """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                logger.debug(
                    f"Making API request to {endpoint} (attempt {attempt + 1})"
                )
                ...
                logger.warning(
                    f"HTTP error on attempt {attempt + 1} for {endpoint}: {e}"
                )
                if attempt == self.max_retries:
                    raise
                delay = 2.0**attempt
    ...
    async def get_play_history(
        self, time_range: int = 30, user_id: int | None = None
    ) -> TautulliData:
        """
    ...
    async def _fetch_paginated_history(
        ...
        logger.info(
            f"Starting paginated fetch for time_range={time_range} days, user_id={user_id}"
        )
        ...
        while api_call_count < max_api_calls and len(all_records) < max_records:
            ...
            try:
                ...
                if api_call_count == 1:
                    first_page_metadata = {
                        k: v for k, v in page_data.items() if k != "data"
                    }
                ...
                if not isinstance(page_records_raw, list):
                    logger.warning(
                        f"Page {api_call_count}: Expected list of records, got {type(page_records_raw)}"
                    )
                    break
                ...
                logger.info(
                    f"Page {api_call_count}: Fetched {records_count} records (offset {start_offset})"
                )
                ...
                if records_count < page_size:
                    logger.info(
                        f"Received {records_count} < {page_size} records, reached end of data"
                    )
                    break

                # Check if we should continue based on time range intelligence
                if self._should_stop_pagination(
                    all_records, time_range, api_call_count
                ):
        ...
        if api_call_count == 1:
            ...
            logger.info(
                f"Pagination complete: fetched {total_records} total records in {api_call_count} API calls"
            )

        # Log performance metrics for monitoring
        if total_records > 2000:
            logger.info(
                f"Large dataset retrieved: {total_records} records for {time_range} day time range"
            )
        elif total_records < 100 and time_range > 7:
            logger.info(
                f"Small dataset: only {total_records} records for {time_range} day time range"
            )
    ...
    def _should_stop_pagination(
        self, all_records: list[dict[str, object]], time_range: int, api_call_count: int
    ) -> bool:
        """
        ...
        if time_range <= 7 and len(all_records) >= 500:
            logger.debug(
                f"Small time range ({time_range} days) with {len(all_records)} records, likely sufficient"
            )
            return True
        ...
        if time_range <= 30 and len(all_records) >= 2000:
            logger.debug(
                f"Medium time range ({time_range} days) with {len(all_records)} records, likely sufficient"
            )
            return True
        ...
        if api_call_count >= 3 and len(all_records) >= 1500:
            logger.debug(
                f"Made {api_call_count} API calls with {len(all_records)} records, stopping to avoid excessive requests"
            )
            return True
    ...
    async def get_plays_per_month(
        self, time_range_months: int = 12, user_id: int | None = None
    ) -> TautulliData:
        """
                ...
                if isinstance(user, dict):
                    ...
                    if user_email == email:
                        ...
                        user_record: UserRecord = {
                            ...
                            if isinstance(user_id_raw, (int, float, str))
                            else 0,
                            "username": str(username_raw)  # pyright: ignore[reportUnknownArgumentType]
                            if username_raw is not None
                            else "",
                            "friendly_name": str(friendly_name_raw)  # pyright: ignore[reportUnknownArgumentType]
                            if friendly_name_raw is not None
                            else "",
                            ...
                            if isinstance(is_active_raw, (int, float, str))
                            else 0,
                        }
                        return user_record
