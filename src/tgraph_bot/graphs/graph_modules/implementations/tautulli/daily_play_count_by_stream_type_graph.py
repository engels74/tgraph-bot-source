"""
Daily play count by stream type graph for TGraph Bot.

This module inherits from BaseGraph and uses the Seaborn library to implement
the logic to plot daily play counts separated by stream type (transcode decision).
This approach allows users to see how much of their media is being direct played,
transcoded, or copied on a daily basis.
"""
...
class DailyPlayCountByStreamTypeGraph(BaseGraph, VisualizationMixin):
    """Graph showing daily play counts separated by stream type (transcode decision)."""
    ...
    def __init__(
        ...
        """
        Initialize the daily play count by stream type graph.
    ...
    def get_title(self) -> str:
        ...
        """
        return self.get_enhanced_title_with_timeframe("Daily Play Count by Stream Type")
    ...
    def generate(self, data: Mapping[str, object]) -> str:
        """
        Generate the daily play count by stream type graph using the provided data.
        ...
        """
        logger.info("Generating daily play count by stream type graph")
        ...
        try:
            ...
            # Step 6: Check if stream type separation is enabled (always for this graph)
            # This graph is specifically designed for stream type separation
            use_separation = True
            ...
            output_path = self.finalize_and_save_figure(
                graph_type="daily_play_count_by_stream_type", user_id=None
            )
            return output_path
    ...
    def _generate_stream_type_separated_visualization(
        self, ax: Axes, processed_records: ProcessedRecords
    ) -> None:
        """
        Generate stream type separated visualization showing different stream types.
        ...
        daily_stream_type_data = self._aggregate_by_date_and_stream_type(
            processed_records, time_range_days
        )

        if not daily_stream_type_data:
        ...
        for stream_type_data in daily_stream_type_data.values():
            all_dates.update(stream_type_data.keys())
        ...
        for stream_type, stream_data in daily_stream_type_data.items():
            if not stream_data:
                continue
        ...
        if all_x_data and all_y_data:
            self.annotation_helper.annotate_line_points(
                ax=ax,
                config_key="graphs.appearance.annotations.enabled_on.daily_play_count_by_stream_type",
                x_data=all_x_data,
                y_data=all_y_data,
                ha="center",
                va="bottom",
        ...
        if self.is_peak_annotations_enabled():
            self._add_peak_annotations(ax, daily_stream_type_data, sorted_dates)

        logger.info(
            f"Created stream type separated daily play count graph with {len(stream_types_plotted)} stream types and {num_dates} data points"
        )

    def _generate_no_stream_data_visualization(self, ax: Axes) -> None:
        """
        Generate visualization when no stream type data is available.
    ...
    def _aggregate_by_date_and_stream_type(
        ...
        """
        Aggregate records by date and stream type with date filling.

        Args:
            records: List of processed play history records
            time_range_days: Number of days for date filling
    ...
    def _add_peak_annotations(
        ...
        """
        Add peak annotations for stream type separated data.
