...
class StartupSequence:
    ...
    async def cleanup_previous_messages(self) -> None:
        ...
        try:
            ...
            if not isinstance(channel, discord.TextChannel):
                logger.warning(f"Channel {config.CHANNEL_ID} is not a text channel or not found")
                return
    ...
    async def post_initial_graphs(self) -> None:
        ...
        try:
            ...
            if not isinstance(channel, discord.TextChannel):
                logger.error(f"Channel {config.CHANNEL_ID} is not a text channel or not found")
                return
    ...
    async def update_scheduler_state(self) -> None:
        ...
        try:
            if not self.initial_post_completed:
                logger.warning("Skipping scheduler state update - no graphs were posted")
                return
