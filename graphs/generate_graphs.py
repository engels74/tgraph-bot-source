...
def generate_graphs(data, folder, current_translations, current_config):
    ...
    if config["ENABLE_DAILY_PLAY_COUNT"]:
        ...
        start_date = end_date - timedelta(
            days=config["TIME_RANGE_DAYS"] - 1
        )  # Include the end date
        dates = [
            start_date + timedelta(days=i) for i in range(config["TIME_RANGE_DAYS"])
        ]
        ...
        plt.title(
            translations["daily_play_count_title"].format(
    ...
    if config["ENABLE_TOP_10_PLATFORMS"]:
        ...
        plt.title(
            translations["top_10_platforms_title"].format(
                days=config["TIME_RANGE_DAYS"]
            ),
            fontweight="bold",
        )
