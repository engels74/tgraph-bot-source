...
class DailyPlayCountGraph(BaseGraph):
    ...
    def plot(self, processed_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Set x-axis ticks to exactly match our data points
            ...
            for serie in processed_data["series"]:
                ...
                if self.config.get("ANNOTATE_DAILY_PLAY_COUNT", False):
                    for date, value in zip(processed_data["dates"], serie["data"]):
                        if value > 0:
            ...
            # Adjust figure size to accommodate all dates
            fig_width = max(14, len(processed_data["dates"]) * 0.5)  # Scale width based on number of dates
            ...
            self.add_legend()
            self.apply_tight_layout(pad=3.0)  # Increased padding to prevent label cutoff
