...
class PlayCountByMonthGraph(BaseGraph):
    ...
    def plot(self, processed_data: Dict[str, Any]):
        ...
        # Plot movie data
        movie_bars = self.ax.bar(index, processed_data["movie_data"], bar_width,
                                 label="Movies", color=self.get_color("Movies"))

        # Plot TV data, stacked on top of movie data
        tv_bars = self.ax.bar(index, processed_data["tv_data"], bar_width,
        ...
        if self.config["ANNOTATE_PLAY_COUNT_BY_MONTH"]:
            for i in range(len(index)):
                ...
                # Annotate movie value
                if movie_value > 0:
                    self.ax.text(i, movie_value/2, str(movie_value), ha='center', va='center')

                # Annotate TV value
                if tv_value > 0:
                    self.ax.text(i, movie_value + tv_value/2, str(tv_value), ha='center', va='center')

                # Annotate total on top
                self.ax.text(i, total, str(total), ha='center', va='bottom')
