    ...
    try:
        ...
        if not path.exists():
            return FileValidationResult(
                valid=False, error_message=f"File does not exist: {file_path}"
            )

        # Check if it's actually a file
        if not path.is_file():
            return FileValidationResult(
                valid=False, error_message=f"Path is not a file: {file_path}"
            )
...
async def upload_files_to_channel(
    ...
    if not file_paths:
        return DiscordUploadResult(
            success=False, error_message="No files provided for upload"
        )
            ...
            if validation.valid:
                ...
                if discord_file:
                    ...
                    validation_errors.append(
                        f"Failed to create Discord file for {file_path}"
                    )
            else:
