...
class TestDefaultPaths:
    ...
    def test_default_values(self) -> None:
        ...
        assert defaults.CONFIG_FILE == Path("config.yml")
        assert defaults.DATA_FOLDER == Path("data")
        assert defaults.LOG_FOLDER == Path("logs")
...
class TestPathValidation:
    ...
    def test_validate_config_file_path_nonexistent_but_valid_parent(self, tmp_path: Path) -> None:
...
class TestArgumentParser:
    ...
    def test_create_argument_parser(self) -> None:
        ...
        assert getattr(config_action, "default", None) == "config.yml"
        assert getattr(data_action, "default", None) == "data"
        assert getattr(log_action, "default", None) == "logs"
...
class TestParseArguments:
    ...
    def test_parse_arguments_home_directory(self, tmp_path: Path) -> None:
        ...
        with patch.object(Path, "expanduser", mock_expanduser):
            args = [
                "--config-file", "~/tgraph/config.yml",
                "--data-folder", "~/tgraph/data",
                "--log-folder", "~/tgraph/logs",
            ]
