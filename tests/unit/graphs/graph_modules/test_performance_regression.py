...
class TestPerformanceRegression:
    ...
    def test_basic_graph_generation_performance(self) -> None:
        ...
        with matplotlib_cleanup():
            ...
            assert generation_time < 2.0, (
                f"Graph generation took {generation_time:.2f}s, expected < 2.0s"
            )
    ...
    def test_complex_graph_generation_performance(self) -> None:
        ...
        with matplotlib_cleanup():
            ...
            assert generation_time < 5.0, (
                f"Complex graph generation took {generation_time:.2f}s, expected < 5.0s"
            )
    ...
    def test_figure_setup_performance(self) -> None:
        ...
        with matplotlib_cleanup():
            ...
            assert setup_time < 0.5, (
                f"Figure setup took {setup_time:.2f}s, expected < 0.5s"
            )
            ...
            assert cleanup_time < 0.1, (
                f"Figure cleanup took {cleanup_time:.2f}s, expected < 0.1s"
            )
    ...
    def test_memory_usage_during_generation(self) -> None:
        ...
        assert memory_used < 50.0, (
            f"Graph generation used {memory_used:.2f}MB, expected < 50MB"
        )
    ...
    def test_configuration_access_performance(self) -> None:
        ...
        assert access_time < 0.1, (
            f"1000 config accesses took {access_time:.2f}s, expected < 0.1s"
        )
    ...
    def test_graph_factory_creation_performance(self) -> None:
        ...
        assert creation_time < 0.1, (
            f"Factory creation took {creation_time:.2f}s, expected < 0.1s"
        )
        ...
        assert retrieval_time < 0.01, (
            f"Graph types retrieval took {retrieval_time:.2f}s, expected < 0.01s"
        )
    ...
    def test_concurrent_graph_generation_performance(self) -> None:
        ...
        if len(results) > 1:
            ...
            assert max_time < avg_time * 2.0, (
                f"Performance degradation detected: max={max_time:.2f}s, avg={avg_time:.2f}s"
            )
    ...
    def test_repeated_setup_cleanup_performance(self) -> None:
        ...
        assert avg_setup_time < 0.1, (
            f"Average setup time {avg_setup_time:.3f}s, expected < 0.1s"
        )
        assert avg_cleanup_time < 0.05, (
            f"Average cleanup time {avg_cleanup_time:.3f}s, expected < 0.05s"
        )
        ...
        assert max_setup_time < 0.2, (
            f"Slowest setup {max_setup_time:.3f}s, expected < 0.2s"
        )
        assert max_cleanup_time < 0.1, (
            f"Slowest cleanup {max_cleanup_time:.3f}s, expected < 0.1s"
        )
    ...
    def test_large_data_processing_performance(self) -> None:
        ...
        with matplotlib_cleanup():
            ...
            assert generation_time < 3.0, (
                f"Large data processing took {generation_time:.2f}s, expected < 3.0s"
            )
    ...
    def test_configuration_validation_performance(self) -> None:
        ...
        assert validation_time < 0.5, (
            f"200 config validations took {validation_time:.2f}s, expected < 0.5s"
        )
    ...
    def test_media_type_processor_performance(self) -> None:
        ...
        assert access_time < 0.01, (
            f"Processor access took {access_time:.3f}s, expected < 0.01s"
        )
        ...
        assert operation_time < 0.1, (
            f"1000 processor ops took {operation_time:.2f}s, expected < 0.1s"
        )
    ...
    def test_memory_efficiency_multiple_graphs(self) -> None:
        ...
        assert memory_used < 100.0, (
            f"Multiple graphs used {memory_used:.2f}MB, expected < 100MB"
        )
    ...
    def test_garbage_collection_efficiency(self) -> None:
        ...
        assert objects_increase < 1000, (
            f"Object count increased by {objects_increase}, expected < 1000"
        )
        ...
        with matplotlib_cleanup():
            ...
            with patch.object(graph, "save_figure") as mock_save_figure:
                ...
                assert generation_time < 1.0, (
                    f"Generation without I/O took {generation_time:.2f}s, expected < 1.0s"
                )
    ...
    def test_resource_cleanup_thoroughness(self) -> None:
        ...
        assert (
            figure_ref is not None
        )  # Reference still exists but should be cleaned by matplotlib
        assert (
            axes_ref is not None
        )  # Reference still exists but should be cleaned by matplotlib
