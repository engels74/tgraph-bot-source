...
""")
    ...
    def test_config_file_structure(self) -> None:
        ...
        assert config["weblate"]["url"] == "https://weblate.engels74.net/"

        # Check for expected components
        all_sections: list[str] = list(config.sections())
        components: list[str] = [
            section for section in all_sections if section.startswith("component ")
        ]
...
class TestWeblateIntegration:
    ...
    def test_filemask_pattern_validity(self) -> None:
        ...
        for section_name in section_names:
            ...
            assert "/" in filemask, (
                f"Filemask should contain path separator: {filemask}"
            )
        ...
        for section_name in section_names:
            ...
            if "locale/messages.pot" in str(template_path):
                assert template_path.exists(), (
                    f"Template file should exist: {template_path}"
                )
            elif "README.md" in str(template_path):
                assert template_path.exists(), (
                    f"README template should exist: {template_path}"
                )
    ...
    def test_repository_configuration(self) -> None:
        ...
        for section_name in section_names:
            ...
            assert "tgraph-bot-source" in repo_url, (
