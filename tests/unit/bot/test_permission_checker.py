...
class TestPermissionChecker:
    ...
    async def test_get_slash_commands_retry_success(self) -> None:
        ...
        with patch('src.tgraph_bot.bot.permission_checker.asyncio.sleep', new_callable=AsyncMock):
            result = await self.permission_checker.get_slash_commands(self.mock_guild)
    ...
    def test_format_permission_table_alignment(self) -> None:
        ...
        for i, line in enumerate(lines):
            ...
            assert actual_visual_width == expected_visual_width, f"Line {i+1} has incorrect visual width: {actual_visual_width} (expected {expected_visual_width})\nLine: '{line}'"

        # Check that all lines have proper border characters
        for i, line in enumerate(lines):
            if i == 0:  # Top border
                assert line.startswith("┌") and line.endswith("┐"), f"Top border line {i+1} malformed: '{line}'"
            elif i == len(lines) - 1:  # Bottom border
                assert line.startswith("└") and line.endswith("┘"), f"Bottom border line {i+1} malformed: '{line}'"
            elif "├" in line:  # Separator lines
                assert line.startswith("├") and line.endswith("┤"), f"Separator line {i+1} malformed: '{line}'"
            else:  # Content lines
                assert line.startswith("│") and line.endswith("│"), f"Content line {i+1} malformed: '{line}'"

        # Verify specific content is present and properly formatted
        assert "🔐 Permission Status for NoobSkateCrew" in result
        assert "🤖 Bot Permissions" in result
        assert "⚡ Slash Command Analysis" in result
        assert "Legend: 🔒=Admin 🔓=User ✅=Has Perms ❌=No Perms 🛡️=Manage Server" in result
    ...
    async def test_security_warnings_based_on_accessibility(self) -> None:
        ...
        # Mock sync status
        with patch.object(self.permission_checker, '_check_command_sync_status', new_callable=AsyncMock) as mock_sync:
            ...
            async def mock_enhanced_analysis(command: MagicMock, guild: MagicMock) -> dict[str, str | list[str]]:  # pyright: ignore[reportUnusedParameter]
            ...
            with patch.object(self.permission_checker, '_analyze_enhanced_command_permissions', side_effect=mock_enhanced_analysis):
                with patch.object(self.permission_checker, '_analyze_integration_permissions', new_callable=AsyncMock) as mock_integration:
                    ...
                    with patch('src.tgraph_bot.bot.permission_checker.logger') as mock_logger:
                        await self.permission_checker.check_slash_command_permissions(self.mock_guild)

                        # Should log security warnings for ALL admin commands
                        warning_calls = [call for call in mock_logger.warning.call_args_list  # pyright: ignore[reportAny]
                        ...
                        config_warning_found = any("/config" in msg for msg in warning_messages)
                        update_graphs_warning_found = any("/update_graphs" in msg for msg in warning_messages)
                        test_scheduler_warning_found = any("/test_scheduler" in msg for msg in warning_messages)
                        
                        assert config_warning_found, "Expected warning for /config command"
                        assert update_graphs_warning_found, "Expected warning for /update_graphs command"
                        assert test_scheduler_warning_found, "Expected warning for /test_scheduler command"
    ...
    async def test_log_permission_status_multiple_guilds(self) -> None:
        ...
        with patch('src.tgraph_bot.bot.permission_checker.logger') as mock_logger:
            ...
            info_calls = [call for call in mock_logger.info.call_args_list   # pyright: ignore[reportAny]
    ...
    async def test_check_slash_command_permissions_combines_global_and_guild(self) -> None:
        ...
        # Setup mocks
        self.permission_checker.check_bot_permissions = AsyncMock(return_value=mock_permissions)

        # Mock fetch_commands to return different results for guild vs global
        async def mock_fetch_commands(guild: discord.Guild | None = None) -> list[MagicMock]:
            if guild is None:
        ...
        with patch.object(self.permission_checker, '_check_command_sync_status', new_callable=AsyncMock) as mock_sync_check:
            ...
            with patch('src.tgraph_bot.bot.permission_checker.logger') as mock_logger:
                ...
                found_availability_log = any("commands available:" in call for call in info_calls)
                assert found_availability_log, f"Expected availability log not found in: {info_calls}"
    ...
    async def test_analyze_enhanced_command_permissions_default_access(self) -> None:
        ...
        result = await self.permission_checker._analyze_enhanced_command_permissions(command, self.mock_guild)  # pyright: ignore[reportPrivateUsage]
    ...
    async def test_analyze_enhanced_command_permissions_with_role_overrides(self) -> None:
    ...
    async def test_analyze_enhanced_command_permissions_with_user_overrides(self) -> None:
    ...
    async def test_analyze_enhanced_command_permissions_with_channel_overrides(self) -> None:
    ...
    async def test_analyze_enhanced_command_permissions_fetch_error(self) -> None:
        ...
        command.fetch_permissions = AsyncMock(side_effect=discord.HTTPException(MagicMock(status=403), "Forbidden"))
    ...
    async def test_analyze_integration_permissions_no_restrictions(self) -> None:
        ...
        result = await self.permission_checker._analyze_integration_permissions(self.mock_guild)  # pyright: ignore[reportPrivateUsage]
    ...
    async def test_analyze_integration_permissions_with_restrictions(self) -> None:
        ...
        mock_guild_permissions = MagicMock()
        mock_guild_permissions.permissions = [mock_role_permission, mock_channel_permission]
    ...
    async def test_analyze_integration_permissions_fetch_error(self) -> None:
        ...
        # Mock API failure
        self.mock_bot.tree.fetch_commands = AsyncMock(side_effect=discord.Forbidden(MagicMock(status=403), "Forbidden"))  # pyright: ignore[reportAny]
        
        result = await self.permission_checker._analyze_integration_permissions(self.mock_guild)  # pyright: ignore[reportPrivateUsage]
        
        assert result["global_roles_access"] == "Unable to fetch (need Manage Server)"
        assert result["global_channels_access"] == "Unable to fetch (need Manage Server)"
    ...
    async def test_check_slash_command_permissions_optimized_api_calls(self) -> None:
        ...
        assert fetch_commands_call_count <= 3, f"Too many API calls: {fetch_commands_call_count} (expected <= 3)"
    ...
    async def test_concurrent_permission_analysis(self) -> None:
        ...
        for i in range(5):  # Create 5 commands to test concurrency
            ...
            mock_cmd.fetch_permissions = AsyncMock(side_effect=delayed_fetch_permissions)
            mock_commands.append(mock_cmd)
        ...
        assert execution_time < 0.05, f"Execution took too long: {execution_time}s (expected < 0.05s)"
    ...
    async def test_cached_command_data_reuse(self) -> None:
        ...
        async def track_fetch_commands(guild: discord.Guild | None = None) -> list[MagicMock]:
        ...
        assert guild_calls == 1, f"Guild commands fetched {guild_calls} times (expected 1)"
        assert global_calls == 1, f"Global commands fetched {global_calls} times (expected 1)"
    ...
    def test_format_enhanced_permission_table_with_integrations(self) -> None:
        ...
        result = self.permission_checker._format_enhanced_permission_table(  # pyright: ignore[reportPrivateUsage]
            self.mock_guild, bot_permissions, enhanced_command_analyses, integration_analysis
        )
