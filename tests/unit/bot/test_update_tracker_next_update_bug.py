...
class TestNextUpdateBugFix:
    ...
    @pytest.fixture
    def update_tracker(self, mock_bot: AsyncMock, temp_state_file: Path) -> UpdateTracker:
    ...
    async def test_next_update_field_updated_after_successful_interval_based_update(
        ...
        config = SchedulingConfig(update_days=1, fixed_update_time="XX:XX")  # Daily interval
        state = ScheduleState()
        state.last_update = base_time - timedelta(days=1)  # Previous update was 1 day ago
        ...
        assert current_state.last_update >= base_time  # Should be at or after the scheduled time
        ...
        assert current_state.next_update > base_time  # Should be AFTER the just-executed time
        
        # For daily interval, next update should be ~1 day later
        expected_next_update = base_time + timedelta(days=1)
        time_diff = abs((current_state.next_update - expected_next_update).total_seconds())
    ...
    async def test_next_update_field_updated_after_successful_fixed_time_update(
        self, update_tracker: UpdateTracker, temp_state_file: Path  # pyright: ignore[reportUnusedParameter]
    ) -> None:
        """
        ...
        base_time = datetime(2025, 7, 27, 14, 30, 0, tzinfo=get_system_timezone())  # 2:30 PM
        
        # Set up the state as if we're about to execute an update
        config = SchedulingConfig(update_days=7, fixed_update_time="14:30")  # Weekly at 2:30 PM
        state = ScheduleState()
        state.last_update = base_time - timedelta(days=7)  # Previous update was 1 week ago
        state.next_update = base_time  # We're about to execute this update
    ...
    async def test_state_persistence_includes_updated_next_update(
        ...
        config = SchedulingConfig(update_days=2, fixed_update_time="XX:XX")  # Every 2 days
        state = ScheduleState()
        ...
        time_diff = abs((loaded_state.next_update - expected_next_update).total_seconds())
        assert time_diff < 60  # Within 1 minute tolerance
    ...
    async def test_bug_reproduction_scenario(
        ...
        expected_next_time = datetime(2025, 7, 28, 0, 5, 0, tzinfo=get_system_timezone())
        
        config = SchedulingConfig(update_days=1, fixed_update_time="00:05")  # Daily at 00:05
        state = ScheduleState()
    ...
    async def test_multiple_consecutive_updates_maintain_correct_next_update(
        ...
        for i in range(3):
            ...
            assert time_diff < 60, f"Update {i+1}: next_update not correctly calculated"
