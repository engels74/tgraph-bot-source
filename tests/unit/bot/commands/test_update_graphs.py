...
class TestUpdateGraphsCog:
    ...
    def test_tgraph_bot_property_with_wrong_bot_type(self) -> None:
        ...
        with patch('src.tgraph_bot.bot.commands.update_graphs.ConfigurationHelper'), \
    ...
    async def test_update_graphs_success_with_cleanup(
        self,
        update_graphs_cog: UpdateGraphsCog
    ) -> None:
        ...
        with patch.object(update_graphs_cog.config_helper, 'validate_discord_channel', return_value=mock_channel), \
             patch.object(update_graphs_cog, '_cleanup_bot_messages', new_callable=AsyncMock) as mock_cleanup, \
             patch('src.tgraph_bot.bot.commands.update_graphs.GraphManager') as mock_graph_manager_class, \
             patch.object(update_graphs_cog, '_post_graphs_to_channel', new_callable=AsyncMock, return_value=3) as mock_post, \
    ...
    async def test_update_graphs_cleanup_failure(
        ...
        with patch.object(update_graphs_cog.config_helper, 'validate_discord_channel', return_value=mock_channel), \
             patch.object(update_graphs_cog, '_cleanup_bot_messages', new_callable=AsyncMock, side_effect=Exception("Cleanup failed")) as mock_cleanup, \
    ...
    async def test_update_graphs_no_graphs_generated(
        ...
        with patch.object(update_graphs_cog.config_helper, 'validate_discord_channel', return_value=mock_channel), \
             patch.object(update_graphs_cog, '_cleanup_bot_messages', new_callable=AsyncMock) as mock_cleanup, \
             patch('src.tgraph_bot.bot.commands.update_graphs.GraphManager') as mock_graph_manager_class, \
             patch.object(update_graphs_cog, '_post_graphs_to_channel', new_callable=AsyncMock) as mock_post, \
             patch('src.tgraph_bot.bot.commands.update_graphs.ProgressCallbackManager'):
    ...
    async def test_cleanup_bot_messages_success(
        ...
        with patch.object(type(update_graphs_cog.bot), 'user', new_callable=lambda: mock_bot_user):
    ...
    async def test_cleanup_bot_messages_rate_limiting(
        ...
        with patch.object(type(update_graphs_cog.bot), 'user', new_callable=lambda: mock_bot_user), \
             patch('src.tgraph_bot.bot.commands.update_graphs.asyncio.sleep', new_callable=AsyncMock) as mock_sleep:
    ...
    async def test_update_graphs_partial_success(
        ...
        with patch.object(update_graphs_cog.config_helper, 'validate_discord_channel', return_value=mock_channel), \
             patch.object(update_graphs_cog, '_cleanup_bot_messages', new_callable=AsyncMock), \
             patch('src.tgraph_bot.bot.commands.update_graphs.GraphManager') as mock_graph_manager_class, \
             patch.object(update_graphs_cog, '_post_graphs_to_channel', new_callable=AsyncMock, return_value=2) as mock_post, \
             patch('src.tgraph_bot.bot.commands.update_graphs.ProgressCallbackManager'):
