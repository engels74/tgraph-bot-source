...
class TestRecoveryManager:
    ...
    def test_detect_missed_updates_no_history(
    ...
    def test_detect_missed_scheduled_update(
        self, recovery_manager: RecoveryManager
    ) -> None:
    ...
    def test_detect_multiple_interval_missed_updates(
        self, recovery_manager: RecoveryManager
    ) -> None:
    ...
    def test_validate_schedule_integrity_valid(
        self, recovery_manager: RecoveryManager
    ) -> None:
    ...
    def test_validate_schedule_integrity_invalid(
        self, recovery_manager: RecoveryManager
    ) -> None:
    ...
    def test_repair_schedule_state(self, recovery_manager: RecoveryManager) -> None:
        ...
        # Check repairs
        assert (
            repaired_state.next_update is not None
            and repaired_state.next_update > current_time
        )  # Fixed future time
    ...
    @pytest.mark.asyncio
    async def test_perform_recovery_with_callback(
        self, recovery_manager: RecoveryManager
    ) -> None:
...
class TestUpdateTrackerRecovery:
    ...
    async def test_recovery_status_reporting(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                isinstance(status["state_file_path"], str)
                and str(state_file) in status["state_file_path"]
            )
