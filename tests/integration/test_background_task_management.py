...
class TestBackgroundTaskManager:
    ...
    @pytest.mark.asyncio
    async def test_task_failure_and_restart(
        self, task_manager: BackgroundTaskManager
    ) -> None:
...
async def test_scheduler_health_updates_during_long_waits() -> None:
    ...
    try:
        ...
        async def mock_wait_with_health_updates(
            total_wait_seconds: float, task_name: str = "update_scheduler"
        ) -> bool:
            ...
            health_update_interval = (
                0.2  # Increased from 0.5 to 0.2 for more predictable timing
            )
            elapsed = 0.0
        ...
        min_expected_time = (
            wait_time * 0.7
        )  # Allow 30% shorter due to async optimizations
        ...
        assert final_health > initial_health, (
            "Health timestamp should have been updated"
        )
