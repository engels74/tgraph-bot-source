...
class TestAsyncThreading:
    ...
    async def test_graph_manager_async_threading(self) -> None:
        ...
        with (
            ...
            def mock_sync_generation(
                _data: dict[str, object], _progress_tracker: object | None = None
            ) -> list[str]:
            ...
            with patch("asyncio.to_thread") as mock_to_thread:
                ...
                with (
                    patch.object(
                        graph_manager, "_generate_graphs_sync", mock_sync_generation
                    ),
                    patch.object(
                        graph_manager,
                        "_validate_generated_files",
                        return_value=["test_graph.png"],
                    ),
                ):
    ...
    async def test_user_graph_manager_async_threading(self) -> None:
        ...
        with (
            ...
            # Mock the synchronous graph generation method
            def mock_sync_user_generation(
                _user_email: str,
                _user_data: dict[str, object],
                _progress_tracker: object | None = None,
            ) -> list[str]:
                return []
    ...
    async def test_event_loop_responsiveness(self) -> None:
        ...
        with (
            ...
            with (
                patch.object(
                    graph_manager, "_generate_graphs_sync", slow_sync_operation
                ),
                patch.object(
                ...
                async with graph_manager:
                    ...
                    assert (
                        counter == 10
                    )  # Counter completed, proving event loop wasn't blocked
